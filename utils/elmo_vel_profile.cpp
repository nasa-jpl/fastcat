...
std::string get_timestamp_filename() {
    ...
    oss << std::put_time(&tm_now, "%Y%m%d_%H%M%S") << "_vel_prof_telem.csv";
    return oss.str();
}

int main(int argc, char** argv) {
    if (argc != 6) {
        std::cerr << "Usage: " << argv[0] << " <config_path> <actuator_name> <accel> <cruise_speed> <cruise_duration>" << std::endl;
    ...
    double cruise_speed = std::atof(argv[4]);
    double cruise_duration = std::atof(argv[5]);
    ...
    if (cruise_duration <= 0) {
        std::cerr << "Error: cruise_duration must be positive" << std::endl;
        return 1;
    }
    ...
    // Write CSV header
    csv_file << "unix_time_s,relative_time_s,cmd_velocity_rad_s,actual_velocity_rad_s,actual_current_A" << std::endl;
    ...
    std::cout << "Profile parameters: accel=" << accel << " rad/s², cruise=" << cruise_speed
              << " rad/s, duration=" << cruise_duration << " s" << std::endl;
    std::cout << "Expected motion duration: " << (accel_time + cruise_duration + decel_time) << " s" << std::endl;
    ...
    while (phase != DONE && !g_shutdown) {
        ...
        switch (phase) {
            case ISSUE: {
                // Issue profile velocity command immediately
                ...
                cmd.actuator_prof_vel_cmd.target_velocity = cruise_speed;
                cmd.actuator_prof_vel_cmd.profile_accel = accel;
                cmd.actuator_prof_vel_cmd.max_duration = cruise_duration;
            ...
            case RUN:
                // Trap is self-driving once issued — do not re-queue PROF_VEL or
                // we restart the ramp from cmd_velocity=0 every tick.
    ...
    std::cout << "\n=== Summary ===" << std::endl;
    std::cout << "Peak cmd velocity:    " << std::fixed << std::setprecision(3) << peak_cmd_velocity << " rad/s" << std::endl;
    std::cout << "Peak actual velocity: " << std::fixed << std::setprecision(3) << peak_actual_velocity
